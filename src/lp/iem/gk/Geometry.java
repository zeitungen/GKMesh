...
public class Geometry
{
	...
	//! construit un repere orthogonal dont la normale est aligne sur un vecteur v1, v2 et v3 sont les 2 tangentes, retourn  vecteur.
	...
	{
	    return ( p1.substractionPoint(p2) ).length();
	}

	// renvoie la distanceSquared entre 2 points
	public static float distanceSquared(Point p1, Point p2 )
	{
	    return (p1.substractionPoint(p2)).lengthSquared();
	}

	//! scalaire * point.
	public static Point productPointFloat( float f,Point p )
	{
	    return p.productFloat(f);
	}
	...
	{
	    return v.dividByFloat(v.length());
	}
	...
	public static float absDot(Normal n1, Vector v2 )
	{
	...
	//! max(0, dot) du produit scalaire de 2 vecteurs.
	public static float zeroDot(Vector v1,Normal v2 )
	{
	    return Math.max( 0.f, dot(v2,v1) );
	}

	//! max(0, dot) du produit scalaire de 2 vecteurs.
	public static float zeroDot(Normal v1, Normal v2 )
	{
	    return Math.max( 0.f, dot( v1, v2 ) );
	}
	...
	{
	    return (float)Math.acos( clamp( v.getZ(), -1.f, 1.f ) );
	}
